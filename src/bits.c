    ...
    enum lc3_bits_mode mode, void *buffer, int len)
{
    *bits = (struct lc3_bits){
    ...
    if (mode == LC3_BITS_MODE_READ) {
        ...
        struct lc3_bits_buffer *bits_buffer = &bits->buffer;

        ac->low  = ac_get(bits_buffer) << 16;
        ac->low |= ac_get(bits_buffer) <<  8;
        ac->low |= ac_get(bits_buffer);

        accu_load(accu, bits_buffer);
    }
}
